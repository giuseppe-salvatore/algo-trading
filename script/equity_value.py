...
def get_equity(api):
    ...
    if account.trading_blocked:
        log.warn('Account is currently restricted from trading.')
...
def buy(stock, current_value, number):
    if stock in portfolio:
        log.info("Bought " + str(number) + " of stocks at " + str(current_value))
    ...
    if has_stock_at_least_shares(stock, number):
        for shares in portfolio[stock]:
            if float(shares["shares"]) > number:
                log.info("Selling " + str(number) + " shares at " + str(shares["value"]))
...
def sell_all_remaining_stocks(stock):
    if stock in portfolio:
        for shares in portfolio[stock]:
            msg = "Selling " + str(shares["shares"]) + " at " + str(shares["value"])
            msg += " for a total cost of " + str(float(shares["shares"]) + float(shares["value"])
            log.info(msg)
        del portfolio[stock]
        del stop_loss[stock]
...
def loss_has_been_detected(stock, curr_value):
    if stock in stop_loss:
        if curr_value < stop_loss[stock]:
            msg="Loss detected at " + str(curr_value) + " stop loss at " + str(stop_loss[stock])
            log.info(msg)
            return True
    return False
...
def print_positions(stock):
    if stock in portfolio:
        for position in portfolio[stock]:
            ...
            log.info("   " + str(shares) + " shares at " + str(value) +
                  " total = " + str(get_two_decimal(shares * value)))
        log.info("   Total value: " + str(get_position_value(stock)))
...
def get_instant_gain_percentage(stock, current_value):
    ...
    if position_value == 0:
        return 0.0
    # print(" Perc calc " + str(get_market_value(stock, current_value)) + " - "
    # + str(position_value) + " = " + str(get_market_value(stock, current_value) - position_value))
...
def play_strategy(time_array, stock_array):
    ...
    for i in range(0, len(time_array)):
        ...
        if dataframe["Stock Values"][i] == dataframe["Actual Min"][i]:
            log.info(str(i) + " " + "{:.2f}".format(curr_stock_value) + " " +
                  str(get_instant_gain_percentage("AAPL", curr_stock_value)) + "% Touching")
            status="touching"
            has_touched=True

        else:
            log.info(str(i) + " " + "{:.2f}".format(curr_stock_value) + " " +
                  str(get_instant_gain_percentage("AAPL", curr_stock_value)) + "% Not Touching")
            status="not_touching"

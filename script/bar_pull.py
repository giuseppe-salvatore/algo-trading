...
def get_minute_bars_in_timeframe(account, db_conn, symbol, time_from, time_to):
    ...
    if len(df) < expected_bar_count:
        print("Missing data, fetching from data provider API (expected: " +
              str(expected_bar_count) + " got: " + str(len(df)) + ")")
        ...
        for day in nsye_dates.index:
            ...
            if len(data) < 390:
                print("WARNING: " + str(390 - len(data)) +
                      " bars missing on " + str(day))
            for elem in data:
...
def fetch_stock_data(account, symbol, date_from, date_to):
    return account.api.get_aggs(symbol, timespan='minute', _from=date_from, to=date_to, multiplier=1)
...
if __name__ == "__main__":
    ...
    #barset = live_account.api.get_aggs('QQQ', timespan='minute', _from='2019-04-15', to='2019-04-16', multiplier=1)
    ...
    #     for elem in barset:
    #         insert_bar(conn, (symbol, str(elem.timestamp)[:19], elem.open, elem.close, elem.high, elem.low, elem.volume))
    #     conn.commit()

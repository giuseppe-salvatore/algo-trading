...
class MarketOrderTest(unittest.TestCase):
    """
    Assessing the functional behavior for Market Orders as standalone objects
    """
    ...
    def test_unknown_order_side_triggers_exception(self):
        """
        ...
        self.assertRaises(ValueError, Order, symbol, quantity, "beep boop", date)
    ...
    def test_generated_order_id_length(self):
        """
        Testing the order id is automatically generated and the expected length
        """
...
class LimitOrderTest(unittest.TestCase):
    ...
    def test_exception_if_missing_limit_price(self):
        ...
        self.assertRaises(
            ValueError, Order, symbol, quantity, "buy", date, flavor="limit"
        )

        self.assertRaises(
            ValueError, Order, symbol, quantity, "sell", date, flavor="limit"
        )

    def test_buy_market_bracket_creates_order_with_both_legs(self):
    ...
    def test_buy_market_bracket_creates_order_with_take_profit_leg(self):
        pass

    def test_buy_market_bracket_creates_order_with_stop_loss_leg(self):
        pass

    def test_buy_limit_bracket_creates_order_with_both_legs(self):
        pass

    def test_buy_stop_bracket_creates_order_with_both_leg(self):
        pass
...
class SimulationTradingPlatformTest(unittest.TestCase):
    ...
    def test_trading_platform_get(self):
    ...
    def test_trading_platform_get_error(self):
        self.assertRaises(ValueError, TradingPlatform.get_trading_platform, "unknown")
    ...
    def test_market_order_creation_and_execution(self):
        ...
        platform.deposit(100000)
        platform.tick(
            "SPY",
            Candle(
                date, {"high": 10, "open": 10, "close": 10, "low": 10, "volume": 1000}
            ),
        )
        id = platform.submit_order(
            symbol=symbol, quantity=quantity, side=side, date=date, flavor="market"
        )
    ...
    def test_market_order_execution_creates_position(self):
        ...
        platform.tick(
            ...
            Candle(
                ...
                {"high": 7, "open": 7, "close": 7, "low": 7, "volume": 1000},
            ),
        )
    ...
    def test_buy_limit_order_triggers_on_high(self):
        ...
        platform.tick(
            ...
            Candle(
                datetime.datetime.now(),
                {"high": 12, "open": 12, "close": 12, "low": 12, "volume": 1000},
            ),
        )
        id = platform.submit_order(
            symbol=symbol,
        ...
        platform.tick(
            ...
            Candle(
                datetime.datetime.now(),
                {"high": 11, "open": 11, "close": 11, "low": 11, "volume": 1000},
            ),
        )
        self.assertTrue(id in platform.active_orders)
        self.assertTrue(id not in platform.executed_orders)
        ...
        platform.tick(
            ...
            Candle(
                datetime.datetime.now(),
                {"high": 10, "open": 10, "close": 10, "low": 10, "volume": 1000},
            ),
        )

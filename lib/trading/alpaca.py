...
class AlpacaTrading:
    def __init__(self, account_type: str = "paper"):
        """
        Instantiating a TradeApiProxy will provide authentication and you will
    ...
    def authenticate(self):
        ...
        instead of conf.secret.ALPACA_PAPER_TRADING_REST_ENDPOINT
        """

        if not (
            self.account_type == "paper" or
            self.account_type == "paper2" or
            self.account_type == "live"
        ):
            msg = (
                "Account type should be either 'paper', 'paper2' or 'live': {}".format(
                    self.account_type
                )
            )
        ...
        log.info(
            "Initialising Alpaca REST API to trade with " +
            self.account_type +
            " account"
        )
        ...
        self.api = alpaca.REST(
            alpaca_api_key, alpaca_secret, alpaca_api_endpoint, api_version="v2"
        )

        log.info(
            "Alpaca REST API " + self.account_type + " account successfully initialised"
        )
    ...
    def check_and_update_stop_loss_orders(self):
        ...
        print(
            "{:>6s}, {:>3s}, {:>5s}, {:>10s}, {:>10s}".format(
                "symbol", "qty", "side", "stop price", "limit price"
            )
        )
    ...
    def update_stop_loss_order(self, id: str, market_val: str, qty: str, pos_side: str):
        ...
        stop = (
            stock_market_val * (1.0 - STOP_LOSS_PERC / 100.0)
            if side == "sell"
            else stock_market_val * (1.0 + STOP_LOSS_PERC / 100.0)
        )
    ...
    def cover_position_with_stop_loss(self, pos):
        ...
        stop = (
            market_val * (1.0 - STOP_LOSS_PERC / 100.0)
            if side == "sell"
            else market_val * (1.0 + STOP_LOSS_PERC / 100.0)
        )
        limit = (
            market_val * (1.0 - STOP_LOSS_PERC / 150000.0)
            if side == "sell"
            else market_val * (1.0 + STOP_LOSS_PERC / 150000.0)
        )

        self.submit_stop_loss_order(
            symbol=pos.symbol, side=side, stop=stop, limit=limit, qty=pos.qty
        )
    ...
    def list_new_orders(self):
        ...
        new_orders = list()
        for order in orders:
            if order.status == "new" or order.status == "replaced":
    ...
    def replace_order(
        ...
        self.api.replace_order(
            id, qty=qty, limit_price=limit, stop_price=stop, time_in_force=tif
        )

    def replace_limit_stop_order_by_id(self, id: str, stop: float, limit: float = None):
        self.replace_order(id=id, stop=stop)

    def replace_limit_stop_order(self, symbol: str, stop: float, limit: float):
        open_orders = self.list_open_orders()
        for order in open_orders:
            if symbol in order.symbol:
                self.replace_order(order.id, limit, stop)

    def submit_braket_order(
        self, symbol: str, limit_target: float, stop: float, limit: float
    ):
        pass

    def submit_stop_loss_order(
        self, symbol: str, side: str, qty: int, stop: float, limit: float
    ):
        ...
        print(
            "{:>6s}, {:>3s}, {:>5s}, {:>10s}, {:>10s}".format(
                symbol, qty, side, stop_str, limit_str
            )
        )

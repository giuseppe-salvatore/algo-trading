...
class AlpacaTrading():
    ...
    def authenticate(self):
        ...
        log.info("Initialising Alpaca REST API to trade with " +
                 self.account_type + " account")

        if self.account_type == 'live':
            log.warn(
                "Using live account, carefull as you are using real money here!!!"
            )
        ...
        self.api = alpaca.REST(alpaca_api_key,
                               alpaca_secret,
                               alpaca_api_endpoint,
                               api_version='v2')

        log.info("Alpaca REST API " + self.account_type +
                 " account successfully initialised")
        ...
        if open_orders is not None:
            ...
            for order in open_orders:
                if order.symbol == pos.symbol:
                    self.update_stop_loss_order(order.id, pos.market_value,
                                                pos.qty, pos.side)
    ...
    def update_stop_loss_order(self, id: str, market_val: str, qty: str,
                               pos_side: str):
    ...
    def cover_position_with_stop_loss(self, pos):
        ...
        self.submit_stop_loss_order(symbol=pos.symbol,
                                    side=side,
                                    stop=stop,
                                    limit=limit,
                                    qty=pos.qty)
    ...
    def replace_order(self,
        ...
        self.api.replace_order(id,
                               qty=qty,
                               limit_price=limit,
                               stop_price=stop,
                               time_in_force=tif)

    def replace_limit_stop_order_by_id(self,
                                       id: str,
                                       stop: float,
                                       limit: float = None):
    ...
    def submit_braket_order(self, symbol: str, limit_target: float,
                            stop: float, limit: float):
        pass

    def submit_stop_loss_order(self, symbol: str, side: str, qty: int,
                               stop: float, limit: float):
    ...
    def fetch_watchlist(self):
        ...
        self._cached_watchlist[
            self._cached_watchlist_id] = self.api.get_watchlist(

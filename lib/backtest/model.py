...
class BacktestParams():
    ...
    def validate_param_set(params):
        ...
        success, msg = BacktestParams.check_param_key("Strategy", params, msg, success)
        success, msg = BacktestParams.check_param_key("Parameter Size", params, msg, success)
        success, msg = BacktestParams.check_param_key("Indicator List", params, msg, success)
        success, msg = BacktestParams.check_param_key("Start Date", params, msg, success)
        success, msg = BacktestParams.check_param_key("End Date", params, msg, success)
        success, msg = BacktestParams.check_param_key("Trading Style", params, msg, success)
        success, msg = BacktestParams.check_param_key("Market Data Provider", params, msg, success)
        ...
        if type(params[param]) is not list:
            success = False
            msg += "{} has to be a list of known indicators".format(
                param
            )
        for elem in params[param]:
            if elem not in BacktestParams.get_available_indicators():
                ...
                msg += "{} can only be {} but {} provided instead".format(
                    param,
                    BacktestParams.get_available_indicators(),
                    params[param]
                )

        # Verifying the start date and end date
        param_key = "Start Date"
        success, msg = BacktestParams.check_date(param_key, params[param_key], msg, success)
        param_key = "End Date"
        success, msg = BacktestParams.check_date(param_key, params[param_key], msg, success)
        ...
        if params[param] not in BacktestParams.get_trading_style():
            ...
            msg += "{} can only be {} but {} provided instead".format(
                param,
                BacktestParams.get_trading_style(),
                params[param]
            )
        ...
        if params[param] not in MarketDataProviderUtils.get_available_providers():
            success = False
            msg += "{} can only be {} but {} provided instead".format(
                param,
                MarketDataProviderUtils.get_available_providers(),
                params[param]
            )
    ...
    def execute(self):
        ...
        for symbol in self.symbols:
            try:
                ...
                self._results.trading_session = strategy.trading_session

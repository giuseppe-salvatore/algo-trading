...
def init_all_stock_targets(api):
    ...
    for stock in watchlist:
        ...
        init_target(stock)
        adjust_stop_target(stock, float(quote.askprice) - float(quote.bidprice))
...
def scale_targets(stock):
    ...
    if targets[stock]["loss"] <= INITIAL_TARGET_LOSS:
        targets[stock]["loss"] = targets[stock]["gain"] - GAP_ON_WIN + (targets[stock]["gain"]/5)
    else:
    ...
    print(stock + " hit the target scaling to gain = " +
          str(targets[stock]["gain"]) + "$, loss = " + str(targets[stock]["loss"]) + "$")
...
def trade_watchlist(api):
    ...
    for stock in watchlist:
        ...
        if found is False:
            ...
            print("Got a quote for " + stock + " not in position so trading it")
            submit_order(api, stock, value_close_to_threshold(quote_value), 'buy')
        else:
